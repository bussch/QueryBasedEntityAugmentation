...
class LinkBertModel(nn.Module):
    ...
    def __init__(self, char_size, config, dataset):
        ...
        self.language_model = LongformerModel.from_pretrained("allenai/longformer-base-4096")
    ...
    def _prepare_sample(self, data, tupID, signals_characteristics):
        ...
        if len(input_ids) > 4094 - self.prefix_seq_len:
            input_ids = input_ids[:4094 - self.prefix_seq_len]
            attribute_ids = attribute_ids[:4094 - self.prefix_seq_len]
            print(f'Truncating tuple {tupID}...')
        ...
        for signal in [x for x in signals_characteristics[0]]:
            ...
            if len(pooling_subwords) == 0:
                ...
                print(f'{signal.keyword}->{original_word} not found in {tupID}')
                continue
            else:
...
class LinkBert(object):
    ...
    def __init__(self, char_size, config, dataset):
        ...
        self.linkyBert = LinkBertModel(char_size, self.config, dataset).to(DEVICE)
        self.loss_fn = nn.MSELoss()

        self.optimizer = torch.optim.Adam(self.linkyBert.parameters(), lr=config['learning_rate'])
    ...
    def predict_raw_terms(self, sender, tupID, data):
        ...
        self.curr_input_ids, self.curr_signal_char_pool, self.attribute_ids = \
            self.linkyBert._prepare_sample(data, tupID,
    ...
    def add_sample_to_buffer(self, sample_x_terms, sample_y):
        ...
        self.sample_buffer_attribute_ids.append(self.attribute_ids)
